...
export default function ProactiveSettingsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0F0F1A] text-[#E4E4F0]">
      <div className="max-w-3xl mx-auto px-4 py-8">
        {/* Header */}
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold">Proactive Settings</h1>
            <p className="text-sm mt-1 text-[#8888A8]">
          ...
          <button
            type="button"
            onClick={() => void load()}
            className="px-3 py-1.5 rounded-lg text-sm transition-colors hover:bg-[#2A2A40] text-[#8888A8] border border-[#2A2A40]"
          >
            Refresh
          </button>
        </div>
        ...
        {/* Error */}
        {error && (
          <div className="mb-4 px-4 py-3 rounded-lg text-sm bg-[#2d1a1a] text-[#f87171]">
            {error}
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="text-center py-16 text-[#8888A8]">
            <p className="text-4xl mb-3">⏰</p>
            <p className="text-sm">No proactive jobs configured on this server.</p>
          </div>
        ) : (
          <div className="space-y-3">
            <p className="text-xs font-semibold uppercase tracking-wider mb-2 text-[#6366F1]">
              Scheduled Jobs
            </p>
            ...
            {/* Quiet hours summary */}
            {quietJob && (
              <div className="mt-6 px-4 py-3 rounded-lg flex items-center gap-3 bg-[#12121F] border border-[#1E1E35]">
                ...
                <div>
                  ...
                  <p className="text-xs mt-0.5 text-[#8888A8]">
                    ...
                    Configure via <code className="text-xs px-1 py-0.5 rounded bg-[#1A1A2E]">nself claw proactive</code> CLI.
                  </p>
                </div>
              </div>
            )}
...
function JobCard({ job, onToggle }: { job: ProactiveJob; onToggle: (type: string, enabled: boolean) => void }) {
  ...
  return (
    <div
      className={`px-4 py-4 rounded-lg bg-[#12121F] border border-[#1E1E35] transition-opacity${job.enabled ? '' : ' opacity-60'}`}
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3 flex-1 min-w-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            {meta?.description && (
              <p className="text-xs mt-0.5 text-[#8888A8]">{meta.description}</p>
            )}
            <div className="flex gap-4 mt-2 flex-wrap">
              <span className="text-xs font-mono text-[#8888A8]">{job.cron_expression}</span>
              {job.next_run_at && (
                <span className="text-xs text-[#8888A8]">
                  next: <span className="text-[#E4E4F0]">{formatDate(job.next_run_at)}</span>
                </span>
              )}
              {job.last_run_at && (
                <span className="text-xs text-[#8888A8]">
                  last: <span className="text-[#E4E4F0]">{formatDate(job.last_run_at)}</span>
                </span>
              )}
              {job.failure_count > 0 && (
        ...
        >
          <span
            className={`inline-block h-5 w-5 rounded-full bg-white shadow transition-transform mt-0.5${job.enabled ? ' translate-x-[22px]' : ' translate-x-[2px]'}`}
          />
        </button>
      </div>
    </div>
